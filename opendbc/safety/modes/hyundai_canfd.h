...
static safety_config hyundai_canfd_init(uint16_t param) {
  ...
  static const CanMsg HYUNDAI_CANFD_LKA_STEER_MSG_LONG_HANDOFF_TX_MSGS[] = {
    HYUNDAI_CANFD_LKA_STEER_MSG_COMMON_TX_MSGS(0, 1)
    HYUNDAI_CANFD_LFA_STEERING_COMMON_TX_MSGS(1)
  ...
  if (hyundai_longitudinal) {
    if (hyundai_canfd_lka_steer_msg) {
      ...
      if (hyundai_canfd_dynamic_handoff) {
        ret = BUILD_SAFETY_CFG(hyundai_canfd_lka_steer_msg_long_rx_checks, HYUNDAI_CANFD_LKA_STEER_MSG_LONG_HANDOFF_TX_MSGS);
      } else {
